  ...
  def index
	...
	@all_categories = "Yes"
  	...
  	if request.post?
		...
		if (@f_cat.empty?) or (@f_cat == "")
			@all_categories = "Yes"
		else
			@all_categories = "No"
		end
		if (@f_acc.empty?) or (@f_acc == "")
			@all_accounts = "Yes"
		else
			@all_accounts = "No"
		end
	...
	if (@all_categories == "Yes")
		if (@all_accounts == "Yes")
			#show Null accounts and categories	
			#flash[:notice] = "Show 'em all!"
			@transactions = Transaction.paginate :page => params[:page], :per_page => 30, :order => 'YEAR(date) DESC, MONTH(date) DESC, DAY(date) DESC, id desc', :conditions => ['transactions.number like ? AND description like ? AND ABS(amount) like ? AND transaction_types_id like ? AND date like ?', @f_num, @f_des, @f_amo, @f_typ, @f_dat ]
		else
			#show null categories
			@transactions = Transaction.paginate :page => params[:page], :per_page => 30, :order => 'YEAR(date) DESC, MONTH(date) DESC, DAY(date) DESC, id desc', :conditions => ['transactions.number like ? AND description like ? AND ABS(amount) like ? AND accounts_id like ? AND transaction_types_id like ? AND date like ?', @f_num, @f_des, @f_amo, @f_acc, @f_typ, @f_dat ]
		end
	else 
		if (@all_accounts == "Yes")
			#show null accounts
			@transactions = Transaction.paginate :page => params[:page], :per_page => 30, :order => 'YEAR(date) DESC, MONTH(date) DESC, DAY(date) DESC, id desc', :conditions => ['transactions.number like ? AND description like ? AND ABS(amount) like ? AND categories_id like ? AND transaction_types_id like ? AND date like ?', @f_num, @f_des, @f_amo, @f_cat, @f_typ, @f_dat ]			
		else
